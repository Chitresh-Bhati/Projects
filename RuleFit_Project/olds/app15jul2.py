...
def explain_prediction_llm(d, params, pred, rules_df, fallback_text):
    ...
    prompt = f"""You are explaining an organ allocation decision model to a non-technical person.
...
The most important rule that influenced this: {top_rule if top_rule else "combined feature weights"}

In exactly 2-3 sentences, explain in plain English why the model made this prediction.
Focus on the most important factor. Do not use technical jargon. Do not mention "rules" or "coefficients".
Write as if explaining to a doctor or ethicist."""
...
def explain_model_learned_llm(rules_df, params, stats, fallback_text):
    ...
    prompt = f"""You are summarising what a preference model learned about a person's organ allocation decisions.

The model was trained on {stats.get('n_decisions', '?')} decisions.
It learned these patterns:
{rules_text}

In 2-3 sentences, summarise in plain English what this person seems to value most 
when deciding between two transplant patients. 
Do not use technical jargon. Do not mention "rules", "coefficients", or "model".
Write as if describing the person's values to a medical ethics committee."""
